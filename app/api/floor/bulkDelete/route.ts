  ...
  try {
    ...
    if (matchedFloors.length === 0) {
      return NextResponse.json(
        {
          success: false,
          message: "None of the provided floor(s) found",
        },
        { status: 400 },
      )
    }
    ...
    if (matchedFloors.length === floorIds.length) {
      const deletableFloors = matchedFloors.filter((floor) =>
      ...
      if (deletableFloors.length > 0) {
        ...
        const blockedRooms: string[] = matchedFloors
          .flatMap((floor) =>
          ...
          return NextResponse.json(
            {
              success: false,
              message:
                "Cannot bulk delete floors because some rooms on the floors have approved bookings",
    ...
    return ApiResponse.success({
      ...
      data: {
        deletedFloors: matchedFloors.map((f) => ({ id: f.id, label: f.label })),
      },
    });
  } catch (err) {
