  ...
  try {
    ...
    if (roomsToProcess.length === 0) {
      return NextResponse.json(
        {
          success: false,
          message: "None of provided room(s) exist",
        },
        { status: 400 }
      );
    }

    if (ids.length !== roomsToProcess.length) {
      return NextResponse.json(
        {
          success: false,
          message: "Some provided room(s) do not actually exist",
        },
        { status: 400 },
      )
    }
    ...
    const deletableRooms = roomsToProcess.filter((room) =>
      room.bookings.every(
        (booking) => booking.status !== BookingStatus.approved
      )
    );
    const blockedRooms = roomsToProcess
      .filter((room) =>
        room.bookings.some(
          (booking) => booking.status === BookingStatus.approved
        )
